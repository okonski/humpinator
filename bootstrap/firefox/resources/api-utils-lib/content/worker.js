...
const WorkerGlobalScope = AsyncEventEmitter.compose({
  ...
  process:function(worker){
    ...
    let contentScriptFile = ('contentScriptFile' in worker) ? worker.contentScriptFile
          : null,
        contentScript = ('contentScript' in worker) ? worker.contentScript : null,        contentScriptContext = ('contentScriptContext' in worker) ? worker.contentScriptContext : null;
;

	if(contentScriptContext){
		Object.defineProperties(this._sandbox, contentScriptContext)
		}
...
exports.Worker = Worker.resolve({constructor:'__initWorker'}).compose({
constructor:function Worker(options){
	var window=options.window;
	//console.log("Worker",this.toString(),this._frame);
